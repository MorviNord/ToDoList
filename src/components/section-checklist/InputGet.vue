<script setup>
import { ref } from 'vue'
import { objList } from './baseList'

const inputText = ref('');
let counter = ref(0);
      

const addToObject = () => {

  let newObject = {
    id: counter.value,
    text: inputText.value,
    isDone: false,
  };

  if (inputText.value.length >= 1) {
    objList.value.push(newObject);
    counter.value++
  }

  inputText.value = '';
}

</script>

<template>
    <section class="input">
      <div class="container">
        <div class="input-wrapper">
          <input 
           class="input-wrapper__item"
           v-model="inputText"
           @keyup.enter="addToObject"
           type="text" 
           placeholder="Запиши в меня..."
           >
          <button class="input-wrapper__button" @click="addToObject">
            <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
              width="16" height="16" viewBox="0 0 1280.000000 1280.000000"
              preserveAspectRatio="xMidYMid meet">
              <metadata>
              Created by potrace 1.15, written by Peter Selinger 2001-2017
              </metadata>
              <g transform="translate(0.000000,1280.000000) scale(0.100000,-0.100000)"
              stroke="none">
              <path d="M5150 12613 c-45 -7 -134 -57 -176 -98 -43 -41 -84 -102 -100 -145
              -5 -14 -11 -952 -14 -2225 l-5 -2200 -2200 -5 c-1256 -3 -2211 -9 -2225 -14
              -80 -29 -171 -114 -215 -200 l-30 -59 0 -1271 0 -1271 28 -56 c16 -31 39 -67
              51 -80 38 -40 85 -79 96 -79 5 0 10 -4 10 -8 0 -4 26 -17 58 -27 56 -20 103
              -20 2245 -23 l2187 -2 0 -2188 c0 -2306 -2 -2206 44 -2291 23 -44 105 -131
              122 -131 8 0 14 -4 14 -10 0 -5 5 -10 11 -10 6 0 25 -6 42 -13 86 -37 82 -37
              1307 -37 880 0 1213 3 1239 12 73 23 121 43 121 51 0 4 6 7 13 7 20 0 102 90
              127 140 12 25 28 54 34 65 8 15 12 612 14 2212 l2 2193 2188 3 c2049 2 2190 3
              2230 20 54 22 120 65 149 97 39 43 43 49 43 60 0 5 5 10 10 10 6 0 10 6 10 13
              0 6 8 26 18 42 16 28 17 111 17 1305 0 1194 -1 1277 -17 1305 -10 17 -18 36
              -18 43 0 6 -4 12 -10 12 -5 0 -10 5 -10 11 0 29 -127 132 -190 155 -14 5 -967
              11 -2220 14 l-2195 5 -5 2190 c-5 1927 -7 2194 -21 2225 -46 106 -105 174
              -194 222 l-60 33 -1250 1 c-687 1 -1261 0 -1275 -3z"/>
              </g>
            </svg>
          </button>
        </div>
      </div>
    </section>
</template>

<style lang="scss">
@use '../../sass/common/vars';

.input {
  margin-bottom: 3rem;
}

.input-wrapper {
    display: flex;
    align-items: center;
    justify-content: center;

    gap: 0.5rem;

        &__item {
            padding: 1.2rem;

            width: 100%;

            border: none;
            outline: none;
            border-radius: 0.5rem;

            background: vars.$minor-color--2;
            color: vars.$white-color;
        }

        &__button {
          display: flex;
          align-items: center;

          padding: 1rem;

          border: none;
          border-radius: 0.5rem;
        
          background: vars.$secondary-color;

          cursor: pointer;

          transition: 0.3s;

              svg {
                fill: vars.$minor-color;
              }

            &:hover {
              background: vars.$secondary-color--hover;
            }
        }
}
</style>